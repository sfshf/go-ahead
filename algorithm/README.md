
- [查找技术](search/)

    - [顺序查找](search/sequential/)

    - [二分法查找](search/dichotomy/)

- [排序技术](sort/)

    - [交换类排序法](sort/exchange/)

        - [冒泡排序法](sort/exchange/bubble/)
        - [快速排序法](sort/exchanged/quick/)

    - [插入类排序法](sort/insert/)

        - [简单插入排序法](sort/insert/simple/)
        - [希尔排序法](sort/insert/shell/)

    - [选择类排序法](sort/select/)

        - [简单选择排序法](sort/select/simple/)
        - [堆排序法](sort/select/heap/)

    - [归并排序](sort/others/merge/)


**参考书籍：**

 - [算法基础 （美）托马斯 H.科尔曼（Thomas H.Cormen）著，王宏志 译](https://book.douban.com/subject/26696971/)
 - [公共基础知识（全国计算机等级考试 二级教程）](http://xuanshu.hep.com.cn/front/book/findBookDetails?bookId=5debe69fb0b2bda7c523c0ca)
